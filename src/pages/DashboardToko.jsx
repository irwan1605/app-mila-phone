...
if (
  ...
  map[skuKey].qty =
    Number(map[skuKey].qty || 0) + qtyMasuk;
...
====================== */
  ...
  const exportDashboardStock = () => {
    try {
      const exportRows = mergedRows.map((item, index) => ({
        NO: index + 1,
      ...
      XLSX.writeFile(workbook, `DETAIL_STOCK_${TOKO_AKTIF}.xlsx`);
    } catch (err) {
      console.error(err);
