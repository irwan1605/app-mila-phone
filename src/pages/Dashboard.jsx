// Dashboard.jsx — REALTIME FIREBASE (UI TETAP SAMA)
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const unsub = listenAllTransaksi((list) => {
      ...
      // FIXED: toko list
      const tokoNames = [...new Set(list.map((r) => r.TOKO).filter(Boolean))];
      setTokoList(tokoNames);

      // daftar sales unik
      const uniqueSales = [...new Set(list.map((r) => r.NAMA_SALES || ""))].filter(Boolean);
  ...
  const filteredData = useMemo(() => {
    ...
    if (filterToko !== "semua") f = f.filter((r) => r.TOKO === filterToko);
    if (filterSales !== "semua") f = f.filter((r) => r.NAMA_SALES === filterSales);
  ...
  const totalOmzet = filteredData.reduce(
    (sum, r) =>
      sum + (Number(r.QTY) || 0) * (Number(r.HARGA) || 0),
    0
  );
  ...
  return (
    <div className="p-4 bg-gray-100 rounded-xl shadow-md">
      ...
      <div className="flex flex-wrap gap-2 items-center mb-4">
        ...
        {filterType !== "semua" && (
          <input type="date" onChange={e => setFilterValue(e.target.value)} className="p-2 border rounded" />
        )}
